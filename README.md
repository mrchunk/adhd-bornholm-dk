...
- **Arrangementer**: now has the September 2026 newsletter's three events — Netværkscafé 18+ (Sept 8, bonfire/outdoor location), ADHD UNGE+ (Sept 11), and Tur til Nature Park (Sept 19, with poster image `site/assets/events/2026-09-19.jpg`) — plus a short newsletter intro/closing note from the board. **By the time this site actually goes live, Sept 8 and Sept 11 will already be in the past** — this is placeholder/example content per the user, not meant to ship as-is. Before publishing, prune past events (or move them to "Fra tidligere arrangementer") and add whatever month's newsletter is current. No past-event photos yet, so "Fra tidligere arrangementer" is still an empty state.
